...
from .models import PhotoProject, PlanProject, Tag, TagProject, ArticleTag
...
class PhotoInline(AdminImageMixin, admin.TabularInline):
    model = PhotoProject
    template = "admin/buildhouse/edit_inline/tabular.html"


class TagTagInline(AdminImageMixin, admin.TabularInline):
    model = Tag
    fields = ('preview_image_url', 'name', 'sort')
...
class ArticleTagInline(AdminImageMixin, admin.TabularInline):
    ...
    def edit_link(self, obj):
        return mark_safe(u'<a href="/admin/buildhouse/articletag/%s/" target="_blank">Редактировать</a>' % obj.pk)
...
class ProjectOptions(AdminImageMixin, admin.ModelAdmin):
    list_display = ('preview_image_url', 'name', 'sq', 'order')
    list_editable = ("sq", 'order')
    list_per_page = 1000
...
admin.site.register(ArticleTag, ArticleOptions)
